...
export default class ModList extends Component<
  ...
  { modList: IMod[], query: {search: string, status: string[]} }
> {
  constructor(props) {
    super(props);
  ...
  render() {
    return (
      ...
      {this.props.user && this.props.user.admin && (<div>
        <Label>Approval Status</Label>
      <FormGroup check>
              <Label check>
                <Input type="checkbox" checked={this.state.query.status.indexOf("approved") !== -1} onChange={(e) => this.addCheck(e, "approved")}/>{' '}
                Approved
              </Label>
            </FormGroup>
      <FormGroup check>
              <Label check>
                <Input type="checkbox" checked={this.state.query.status.indexOf("pending") !== -1} onChange={(e) => this.addCheck(e, "pending")}/>{' '}
                Pending
              </Label>
            </FormGroup>
      <FormGroup check>
              <Label check>
                <Input type="checkbox" checked={this.state.query.status.indexOf("declined") !== -1} onChange={(e) => this.addCheck(e, "declined")}/>{' '}
                Declined
              </Label>
            </FormGroup></div>)}
        <div className="mods">
