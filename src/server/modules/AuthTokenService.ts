...
export default class AuthTokenService {
  ...
  public async validateTokens(
    ...
    if (!currentToken) {
      console.debug(
        'No unexpired refresh token for session',
        payload.sessionId
      );
      return null;
    }
    ...
    const secondsSinceNewToken =
      (new Date().getTime() - currentToken.issued.getTime()) / 1000;
    if (secondsSinceNewToken > this.softAccessTokenLifetime) {
      console.debug(
        'Current refresh token older than soft token lifetime',
        secondsSinceNewToken,
        this.softAccessTokenLifetime
      );
      return null;
    }
    ...
    if (
      refreshTokenId !== currentToken.previousId &&
      refreshTokenId !== currentToken.refreshTokenId
    ) {
      console.debug(
        'Refresh token mismatch',
        refreshTokenId,
        currentToken.previousId,
        currentToken.refreshTokenId
      );
      return null;
    }
