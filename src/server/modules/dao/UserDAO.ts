...
export interface IUserDAO extends IBaseDAO<IRedactedDbUser> {
    getWithPassword(filter: dynamic, options?: FindOneOptions): Promise<IDbUser | null>;
}

export default class UserDAO extends BaseDAO<IRedactedDbUser> implements IUserDAO {
    constructor(db: Db) {
        super("user", db);
    }

    public async getWithPassword(filter: dynamic, options?: FindOneOptions) {
        return await this.collection.findOne<IDbUser & { _id: Id }>(filter, options);
    }
